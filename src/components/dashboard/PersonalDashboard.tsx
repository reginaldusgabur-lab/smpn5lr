...
import { CalendarOff, LogIn, LogOut, FileText } from 'lucide-react';
...
export default function PersonalDashboard({ showChart = false }: PersonalDashboardProps) {
  ...
  const { data: monthlyData, isLoading: isMonthlyLoading } = useQuery<{ attendance: DocumentData[], leaves: DocumentData[] } | undefined>({
    ...
    enabled: !!user && !!firestore && showChart,
  });
  ...
  const isLoading = isAuthLoading || isUserDataLoading || isConfigLoading || isAttendanceLoading || isPendingLeaveLoading || (isMonthlyLoading && showChart);

  // --- Data Processing (Memos) ---

  const attendanceChartData = useMemo(() => {
    if (!monthlyData || !schoolConfig || !showChart) return [];
