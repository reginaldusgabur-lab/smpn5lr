...
import { Loader2, CalendarOff, LogIn, LogOut, FileText } from 'lucide-react';
import { useUser, useFirestore } from '@/firebase';
import { doc, collection, query, where, Timestamp, orderBy, getDocs, getDoc, type DocumentData } from 'firebase/firestore';
import { format, isBefore, eachDayOfInterval, startOfMonth, endOfMonth, startOfDay, endOfDay } from 'date-fns';
...
export default function GuruDashboardPage() {
  ...
  const { data: monthlyData, isLoading: isMonthlyLoading } = useQuery<{ attendance: DocumentData[], leaves: DocumentData[] } | undefined>({
    queryKey: ['monthlySummary', user?.uid, format(new Date(), 'yyyy-MM')],
    queryFn: async () => {
        ...
        const [attendance, leaves] = await Promise.all([
             ...
             // This query fetches all leave requests that could possibly overlap with the current month.
             // It gets all leaves that end on or after the first day of the current month.
             // The logic in `attendanceChartData` will then correctly filter and count only the relevant days.
  ...
  const isLoading = isAuthLoading || isUserDataLoading || isConfigLoading || isAttendanceLoading || isMonthlyLoading || isPendingLeaveLoading;
  ...
  const attendanceChartData = useMemo(() => {
    ...
    return [
        ...
        { name: 'Terlambat', total: terlambat },
           ...
           {attendanceChartData && attendanceChartData.length > 0 ? (
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Belum ada data kehadiran untuk ditampilkan di bulan ini.</CardDescription>
                    </CardHeader>
